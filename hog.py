...
def free_bacon(opponent_score):
    ...
    # BEGIN PROBLEM 2
    score = 0
    first_digit = int(opponent_score / 10)
    second_digit = opponent_score % 10 
    score = score + (1+ max(first_digit, second_digit))
    return score
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    while score0 < goal and score1 < goal:
        ...
        if is_perfect_piggy(turn_score1) == True: # Perfect Piggy
                dice_swapped = not dice_swapped
        player = other(player)
        if is_swap(score1, score0) == True: # Swine Swap
...
def check_strategy(strategy, goal=GOAL_SCORE):
    ...
    """
    # BEGIN PROBLEM 7
    for score in range (0, 120):
        for opponent_score in range(0, 120):
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    avg_score = averaged_dice(1, dice)
    result = 1
    for i in range (1, 11):
        roll = averaged_dice(i, dice)
        if roll > avg_score:
            avg_score = roll
            result = i
    return result
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_prime(value):
    	value = next_prime(value)
    	score += value 
    	if is_swap(value, opponent_score) == True and score < opponent_score:
    		return 0
    	elif value >= margin and is_swap(value, opponent_score) == False:
    		return 0
    	else:
    		return num_rolls
    else:
...
def final_strategy(score, opponent_score):
    ...
    The if loop calls 10 rolls to increase the likelihood of getting a turn score
    of one, swapping it with the opponent. The for loop takes the 1/6 chance of 
    rolling i on one dice roll to swap the score with the opponent.

    """
    # BEGIN PROBLEM 12

    num_rolls = 4
    if is_swap(score + 1, opponent_score) and score < opponent_score:
    	return 10
    for i in range(6):
    	if is_swap(score + i + 1, opponent_score) and score + i + 1 < opponent_score:
    		return 1
    if score - opponent_score >= 29:
    	margin = 7
    	num_rolls = 4
    	return num_rolls
